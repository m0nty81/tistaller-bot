...
for i in $(seq 0 $((APP_COUNT-1))); do
    ...
    if [[ -f "$APK_PATH" ]]; then
        ...
        if [[ "$CMP_RESULT" == "2" ]]; then
            # Новая версия < старой - пропускаем полностью (без уведомления)
            log "  Пропущено: версия понижается ($OLD_VER_DISPLAY → $NEW_VER)"
            continue
        elif [[ "$CMP_RESULT" == "0" ]]; then
            # Версии равны (но хэш разный - пересборка) - не обновляем и не уведомляем
            log "  Пропущено: версии равны ($OLD_VER_DISPLAY), хэш разный (пересборка)"
            continue
        else
            # Новая версия > старой
        ...
        # Отправляем уведомление ТОЛЬКО о реальном обновлении
